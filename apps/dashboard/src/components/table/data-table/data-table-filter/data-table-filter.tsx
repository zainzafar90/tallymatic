...
export const DataTableFilter = ({ filters, prefix }: DataTableFilterProps) => {
  ...
  const [activeFilters, setActiveFilters] = useState(getInitialFilters({ searchParams, filters, prefix }));
      ...
      <div className="max-w-2/3 flex flex-wrap items-center gap-2">
        {activeFilters.map((filter) => {
          switch (filter.type) {
            ...
            case 'date':
              return <DateFilter key={filter.key} filter={filter} prefix={prefix} openOnMount={filter.openOnMount} />;
            case 'string':
              return <StringFilter key={filter.key} filter={filter} prefix={prefix} openOnMount={filter.openOnMount} />;
            case 'number':
              return <NumberFilter key={filter.key} filter={filter} prefix={prefix} openOnMount={filter.openOnMount} />;
            default:
              break;
          }
        })}
        ...
        {activeFilters.length > 0 && <ClearAllFilters filters={filters} prefix={prefix} />}
      </div>
    </DataTableFilterContext.Provider>
  );
};
