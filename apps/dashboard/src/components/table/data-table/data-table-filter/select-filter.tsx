...
export const SelectFilter = ({ filter, prefix, multiple, searchable, options, openOnMount }: SelectFilterProps) => {
  ...
  const labelValues = currentValue.map((v) => options.find((o) => o.value === v)?.label).filter(Boolean) as string[];
  ...
  return (
    <Popover.Root modal open={open} onOpenChange={handleOpenChange}>
      <SelectDisplay label={label} value={labelValues} onRemove={handleRemove} />
      <Popover.Portal>
        <Popover.Content
          hideWhenDetached
          align="start"
        ...
        >
          <Command className="h-full">
            ...
            <Command.Empty className="txt-compact-small flex items-center justify-center p-1">
              <span className="w-full px-2 py-1 text-center">No results</span>
            </Command.Empty>
...
export const SelectDisplay = ({ label, value, onRemove }: SelectDisplayProps) => {
